    ...
    <section class="p-4 flex flex-col gap-4">
        ...
        <div class="flex flex-col gap-4 text-xl col-span-4 border border-blue-500 p-4 rounded-lg bg-blue-500/20">
            ...
            <div>
                ...
                <p>{{ $borrow->created_at->format('l, j F Y H:i') }}</p>
            </div>
            <div>
                <h2 class="font-bold text-blue-800">Expected Return Date :</h2>
                <p>{{ $borrow->returned_at->format('l, j F Y H:i') }}</p>
            </div>
            <div>
                <h2 class="font-bold text-blue-800">Status :</h2>
                @if ($borrow->status === 'borrowed' && $borrow->returned_at >= now())
                    <span class="bg-yellow-500 p-2 rounded-md text-yellow-800">Borrowing</span>
                @elseif ($borrow->status === 'borrowed' && $borrow->returned_at->isPast())
                    <span class="bg-amber-700 p-2 rounded-md text-white">Overdue</span>
                @elseif ($borrow->status === 'returned')
                    <span class="bg-green-500 p-2 rounded-md text-green-900">Returned</span>
                @elseif ($borrow->status === 'lost')
                    <span class="bg-red-500 p-2 rounded-md text-red-900">Lost</span>
                @endif
            </div>
        </div>
