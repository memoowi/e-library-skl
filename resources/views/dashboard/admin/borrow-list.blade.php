...
@section('content')
    <main class="p-4">
        <h1 class="text-4xl font-bold text-blue-800 mb-4">Borrow List</h1>

        <table class="p-4 border border-black w-full">
            <thead>
                    ...
                    <tr class="border-b border-neutral-500 hover:bg-neutral-200">
                        ...
                        <td class="p-2">
                            @if ($borrow->status === 'borrowed' && $borrow->returned_at >= now())
                                <span class="text-yellow-500">Borrowing</span>
                            @elseif ($borrow->status === 'borrowed' && $borrow->returned_at->isPast())
                                <span class="text-amber-700">Overdue</span>
                        ...
                        <td class="p-2">{{ $borrow->created_at->format('l, j F Y H:i') }}</td>
                        <td class="p-2">{{ $borrow->returned_at->format('l, j F Y H:i') }}</td>
                        <td class="p-2 flex gap-2">
                            <a href={{ route('dashboard.show-borrow', $borrow->id) }}
                                class="p-2 bg-blue-800 text-white rounded-md w-fit font-medium">Detail</a>
                            ...
                            @if ($borrow->status === 'borrowed' || $borrow->status === 'lost')
                                <form action={{ route('borrow.return') }} method="post">
                                    ...
                                    <button type="submit"
                                        class="p-2 bg-green-600 text-white rounded-md w-fit font-medium">Return</button>
                                </form>
                                ...
                                @if ($borrow->status === 'borrowed')
                                    <form action={{ route('borrow.lost') }} method="post">
                                        ...
                                        <input type="hidden" name="id" value={{ $borrow->id }}>
                                        <button type="submit"
                                            class="p-2 bg-red-800 text-white rounded-md w-fit font-medium">Lost</button>
                                    </form>
                                @endif
                            @endif
        ...
        <div class="flex flex-col gap-4 justify-center items-center mt-2">
            <p>
